...
function _tct_tns_probe() {
    ...
    # 🚀 AWK 去骨拼接與雙緩衝引擎 (The Deboning Engine)
    local parsed_params
        ...
        }
        {
            ...
            if (line ~ /^-+[a-zA-Z0-9@?]/) {
                ...
                if (split_idx > 0) {
                    ...
                    # 物理去骨：整行沒有大間距，代表「只有骨頭沒有肉」
                    flag = line
                    desc = ""
                }
                
                # 狀態機：如果沒有肉，存入懸空記憶體，直接等下一行
                if (desc == "") {
                    pending_flag = flag
                    next
                } else {
            ...
            # 🟢 肉塊拼接：如果有懸空的骨頭，且這行是縮排的說明文字
            else if (pending_flag != "" && orig_line ~ /^[ \t]+/) {
                flag = pending_flag
                desc = line
                pending_flag = ""
            } 
            # 🟢 閘門 B (特徵符號解析，專治 pkg 等不規則排版)
            else {
                pending_flag = ""
                idx = index(line, " - ")
